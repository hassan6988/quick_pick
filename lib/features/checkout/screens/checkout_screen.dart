...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> initCall() async {
    ...
    if (widget.storeId == null) {
      ...
      if (_cartList != null && _cartList!.isNotEmpty) {
        final uniqueStoreIds = _cartList!.map((e) => e?.item?.storeId).whereType<int>().toSet().toList();
        await Get.find<CheckoutController>().initMultipleCheckoutData(uniqueStoreIds);
      }
    }
    if (widget.storeId != null) {
    ...
    Get.find<CheckoutController>().tipController.text =
        Get.find<CheckoutController>().selectedTips != -1 ? AppConstants.tips[Get.find<CheckoutController>().selectedTips] : '';
  }
  ...
  void _setSinglePaymentActive() {
    if ((!_firstTimeCheckPayment &&
            ...
            Get.find<SplashController>().configModel!.activePaymentMethodList!.length == 1) &&
        ((!_isWalletActive && AuthHelper.isLoggedIn()) || !AuthHelper.isLoggedIn())) {
      Future.delayed(const Duration(milliseconds: 600), () {
        Get.find<CheckoutController>().setPaymentMethod(2, isUpdate: false);
        Get.find<CheckoutController>()
            .changeDigitalPaymentName(Get.find<SplashController>().configModel!.activePaymentMethodList![0].getWay!, willUpdate: false);
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn()
          ? GetBuilder<CheckoutController>(builder: (checkoutController) {
              List<DropdownItem<int>> addressList =
                  _getDropdownAddressList(context: context, addressList: Get.find<AddressController>().addressList, store: checkoutController.store);
              ...
              _isOfflinePaymentActive = Get.find<SplashController>().configModel!.offlinePaymentStatus! &&
                  _checkZoneOfflinePaymentOnOff(addressModel: AddressHelper.getUserAddressFromSharedPref(), checkoutController: checkoutController);
              ...
              return GetBuilder<CouponController>(builder: (couponController) {
                ...
                double additionalCharge =
                    Get.find<SplashController>().configModel!.additionalChargeStatus! ? Get.find<SplashController>().configModel!.additionCharge! : 0;
                ...
                final result = calculateDeliveryChargeForMultipleStores(
                  storeList: checkoutController.storeList,
                  address: AddressHelper.getUserAddressFromSharedPref()!,
                  distance: checkoutController.distance,
                  extraCharge: checkoutController.extraCharge,
                  orderAmount: orderAmount ?? 0.0,
                  orderType: checkoutController.orderType!,
                );

                print('Total Delivery Fee: ${result.totalCharge}');
                for (var storeCharge in result.storeCharges) {
                  print('${storeCharge.storeName} (ID: ${storeCharge.storeId}) → Fee: ${storeCharge.deliveryCharge}');
                }

                double deliveryCharge = result.totalCharge;
                if (checkoutController.orderType != 'take_away' && checkoutController.storeList.isNotEmpty) {
                  _deliveryChargeForView = (checkoutController.orderType == 'delivery' ? allStoresHaveFreeDelivery(checkoutController) : true)
                      ? 'free'.tr
                      : deliveryCharge != -1
                ...
                checkoutController
                    .setTotalAmount(total - (checkoutController.isPartialPay ? Get.find<ProfileController>().userInfoModel!.walletBalance! : 0));
                ...
                return (checkoutController.distance != null && checkoutController.store != null)
                    ? Column(
                        children: [
                                      ...
                                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        Expanded(
                                            ...
                                            child: TopSection(
                                              ...
                                              variationPrice: isPassedVariationPrice ? variations : 0,
                                      ...
                                      Padding(
                                        ...
                                        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                          ...
                                          PriceConverter.convertAnimationPrice(
                                            ...
                                            textStyle:
                                                robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(context).primaryColor),
                                          ),
                                        ]),
                                      ),
                                      _orderPlaceButton(
  ...
  Widget _orderPlaceButton(CheckoutController checkoutController, bool todayClosed, bool tomorrowClosed, double orderAmount, double? deliveryCharge,
      double tax, double? discount, double total, double? maxCodOrderAmount, bool isPrescriptionRequired) {
    return Container(
      width: Dimensions.webMaxWidth,
                ...
                ? () {
                    ...
                    if (isGuestLogIn && checkoutController.guestAddress == null && checkoutController.orderType != 'take_away') {
                      ...
                      showCustomSnackBar('enter_confirm_password'.tr);
                        ...
                        (guestPasswordController.text != guestConfirmPasswordController.text)) {
                      ...
                      if (widget.storeId == null) {
                        ...
                        if (checkoutController.paymentMethodIndex == 3) {
                          ...
                          checkoutController.placeOrder(placeOrderBody, checkoutController.store!.zoneId, total, maxCodOrderAmount, widget.fromCart,
                        ...
                        checkoutController.placePrescriptionOrder(
                            ...
                            (checkoutController.orderType == 'take_away' || checkoutController.tipController.text == 'not_now')
                                ? ''
                                : checkoutController.tipController.text.trim(),
  ...
  double _calculateVariationPrice(
      {required Store? store, required List<CartModel?>? cartList, bool calculateDiscount = false, bool calculateWithoutDiscount = false}) {
  ...
  double _calculateDiscount({required Store? store, required List<CartModel?>? cartList, required double price, required double addOns}) {
    ...
    if (store != null && cartList != null) {
      for (var cartModel in cartList) {
        double? dis = (store.discount != null && DateConverter.isAvailable(store.discount!.startTime, store.discount!.endTime)) &&
                cartModel!.item!.flashSale != 1
            ? store.discount!.discount
            : cartModel!.item!.discount;
        String? disType = (store.discount != null && DateConverter.isAvailable(store.discount!.startTime, store.discount!.endTime)) &&
                cartModel.item!.flashSale != 1
            ? 'percent'
  ...
  double _calculateOriginalDeliveryCharge(
      {required Store? store, required AddressModel address, required double? distance, required double? extraCharge}) {
  ...
  bool allStoresHaveFreeDelivery(CheckoutController checkout) {
    if (checkout.storeList.isEmpty) return false;
    return checkout.storeList.every((store) => store?.freeDelivery == true);
  }
  ...
  double _calculateDeliveryCharge(
    ...
    if (orderType == 'take_away' ||
        ...
        (configModel?.adminFreeDelivery?.status == true &&
            (configModel?.adminFreeDelivery?.type != null && configModel?.adminFreeDelivery?.type == 'free_delivery_to_all_store')) ||
        (configModel?.adminFreeDelivery?.status == true &&
            (configModel?.adminFreeDelivery?.type != null && configModel?.adminFreeDelivery?.type == 'free_delivery_by_order_amount') &&
            (configModel!.adminFreeDelivery?.freeDeliveryOver != null && orderAmount >= configModel.adminFreeDelivery!.freeDeliveryOver!)) ||
        Get.find<CouponController>().freeDelivery ||
        (AuthHelper.isGuestLoggedIn() &&
            (Get.find<CheckoutController>().guestAddress == null && Get.find<CheckoutController>().orderType != 'take_away'))) {
      deliveryCharge = 0;
    }

    print("this is delivery charges ${deliveryCharge}");
    print("this is delivery charges ${PriceConverter.toFixed(deliveryCharge)}");
    return PriceConverter.toFixed(deliveryCharge);
  }
  //
  // double _calculateDeliveryChargeForMultipleStores({
  ...
  //
  //     bool isFreeDelivery = orderType == 'take_away' ||
  //         store.freeDelivery == true ||
  //         (configModel?.adminFreeDelivery?.status == true && configModel?.adminFreeDelivery?.type == 'free_delivery_to_all_store') ||
  //         (configModel?.adminFreeDelivery?.status == true &&
  //             configModel?.adminFreeDelivery?.type == 'free_delivery_by_order_amount' &&
  //             (configModel?.adminFreeDelivery?.freeDeliveryOver != null && orderAmount >= configModel!.adminFreeDelivery!.freeDeliveryOver!)) ||
  //         Get
  //             .find<CouponController>()
  //             .freeDelivery ||
  //         (AuthHelper.isGuestLoggedIn() && Get
  //             .find<CheckoutController>()
  //             .guestAddress == null && orderType != 'take_away');
  //
  //     if (!isFreeDelivery) {
  //       double charge = _calculateOriginalDeliveryCharge(
  //         store: store,
  //         address: address,
  //         distance: distance,
  ...
  //     }
  //   }
  //   return PriceConverter.toFixed(totalCharge);
  // }

  DeliveryChargeResult calculateDeliveryChargeForMultipleStores({
    required List<Store?> storeList,
    required AddressModel address,
    required double? distance,
    required double? extraCharge,
    ...
    List<StoreDeliveryChargeModel> storeCharges = [];
    double totalCharge = 0;
    ...
    for (Store? store in storeList) {
      ...
      bool isFreeDelivery = orderType == 'take_away' ||
          ...
          (configModel?.adminFreeDelivery?.status == true &&
              ...
              (configModel?.adminFreeDelivery?.freeDeliveryOver != null &&
                  orderAmount >= (configModel?.adminFreeDelivery?.freeDeliveryOver ?? double.infinity))) ||
          Get.find<CouponController>().freeDelivery ||
          (AuthHelper.isGuestLoggedIn() && Get.find<CheckoutController>().guestAddress == null && orderType != 'take_away');

      double charge = 0;
      if (!isFreeDelivery) {
        charge = _calculateOriginalDeliveryCharge(
          store: store,
          address: address,
          distance: distance,
          extraCharge: extraCharge,
      ...
      storeCharges.add(StoreDeliveryChargeModel(
        storeId: store.id!,
        storeName: store.name ?? 'Unknown',
        deliveryCharge: PriceConverter.toFixed(charge),
      ));
    }

    return DeliveryChargeResult(
      storeCharges: storeCharges,
      totalCharge: PriceConverter.toFixed(totalCharge),
    );
  }
...
class DeliveryChargeResult {
  final List<StoreDeliveryChargeModel> storeCharges;
  final double totalCharge;

  DeliveryChargeResult({
    required this.storeCharges,
    required this.totalCharge,
  });
}

class StoreDeliveryChargeModel {
  final int storeId;
  final String storeName;
  final double deliveryCharge;

  StoreDeliveryChargeModel({
  ...
  @override
  String toString() => 'StoreDeliveryChargeModel(id: $storeId, name: $storeName, charge: $deliveryCharge)';
}
