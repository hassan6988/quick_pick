...
class CheckoutScreenState extends State<CheckoutScreen> {
      ...
      if(widget.storeId == null){
        ...
        if(_cartList != null && _cartList!.isNotEmpty) {
          for(var cart in _cartList!){
            Get.find<CheckoutController>().initCheckoutData(cart?.item?.storeId);
          }
  ...
  double _calculateTotal({required double subTotal, required double deliveryCharge, required double discount, required double couponDiscount, required bool taxIncluded, required double tax, required String orderType, required double tips, required double additionalCharge, required double extraPackagingCharge,}) {
  ...
  Map<int, List<CartModel>> _groupCartItemsByStore(List<CartModel> cartList) {
    Map<int, List<CartModel>> groupedItems = {};

    for (CartModel item in cartList) {
      int storeId = item.item!.storeId!;
